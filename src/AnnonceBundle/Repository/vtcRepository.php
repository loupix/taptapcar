 ...
 */
class vtcRepository extends \Doctrine\ORM\EntityRepository
{



	public function genQuery($params, $lat, $lon){
		$query = $this->createQueryBuilder("vtc")
			->join("vtc.annonce", "ann")
		...
		if(!is_null($params['wifi']))
			$query->andWhere($query->expr()->eq("vtc.wifi", var_export($params['wifi'], true)));
		if(!is_null($params['facture']))
			$query->andWhere($query->expr()->eq("vtc.facture", var_export($params['facture'], true)));
		if(!is_null($params['siegeBebe']))
			$query->andWhere($query->expr()->eq("vtc.siegeBebe", var_export($params['siegeBebe'], true)));
		if(!is_null($params['animaux']))
			$query->andWhere($query->expr()->eq("vtc.animaux", var_export($params['animaux'], true)));
		if(!is_null($params['paiement']))
			$query->andWhere($query->expr()->like("vtc.paiements", "'%".$params['paiement']."%'"));


		// if(!is_null($params['note']) && $params['note'])
		// 	$query->leftJoin('vtc.annonce','ann')->leftJoin("ann.auteur", 'user')->leftJoin('user.clientAvis','avis')
		// 		->having($query->expr()->gte($query->expr()->avg('avis.note'), $params['note']))
		// 		->addGroupBy("avis.id");

		// if(!is_null($params['note']) && $params['note']){

		// 	$qb = $this->createQueryBuilder("v2")
		// 	->leftJoin('v2.annonce', 'ann')->leftJoin('ann.auteur', 'user')->leftJoin('user.clientAvis', 'avis');
		// 	$qb->addSelect($qb-expr()->avg('avis.note'));
		...
		if(!is_null($params['prix']) && $params['prix']){
			switch ($params['prix']) {
				case 'bas':
					$query->andWhere($query->expr()->lte("vtc.tarifJour", "100"));
					break;

				case 'moyen':
					$query->andWhere($query->expr()->lte("vtc.tarifJour", "800"));
					break;

				case 'haut':
					$query->andWhere($query->expr()->gte("vtc.tarifJour", "800"));
					break;
		...
		// 	$query->expr()->gte("vtc.heureDebut", date("'H:i:s'")),
		// 	$query->expr()->lte("vtc.heureFin", date("'H:i:s'"))
		// ));


		// if(!is_null($params['_tri']['heure']) && $params['_tri']['heure'])
		// 	$query->addOrderBy("vtc.date", $params['_tri']['heure']);

		if(!is_null($params['_tri']['prix']) && $params['_tri']['prix'])
			$query->addOrderBy("vtc.tarifJour", $params['_tri']['prix']);

		$query->andWhere("ann.actif=1")->addOrderBy("vtc.urgent", "ASC");
	...
	public function countAll($query){
		return $query->select("COUNT(vtc.id)")
	...
	public function isDisponnible($id, $datetime){
		$query = $this->createQueryBuilder("vtc");
		$value = $query->join("vtc.annonce", "ann")->join("ann.reservations", "res")
			->select("COUNT(res.id)")
			->where($query->expr()->eq("vtc.id", $id))
		...
		$value = $query
			...
			->where($query->expr()->andX(
				$query->expr()->gte("vtc.heureDebut", "'"+$datetime->format("H:i:s")+"'"),
				$query->expr()->lte("vtc.heureFin", "'"+$datetime->format("H:i:s")+"'"),
				$query->expr()->eq("vtc.id", $id)
