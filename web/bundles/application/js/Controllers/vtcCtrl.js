...
myApp.controller("vtcRechercheCtrl", ['$scope','$http','$location', 'VtcService', 'UserService', function($scope, $http, $location, VtcService, UserService){
    // Pagination
  $scope.totalItems = 0;
  $scope.currentPage = 0;
  $scope.maxSize = 5;
  ...
  $scope.loadRecherche = function(){
    ...
    UserService.getPosition().then(function(position){
      ...
      VtcService.find($scope.recherche).then(function(response){
        ...
        for(var i=0;i<$scope.annonces.length;i++){
          var view = Routing.generate('annonce_vtc_get', { 'id': $scope.annonces[i].Id });
...
.controller("vtcAnnonceCtrl", ['$scope','$http', '$location', 'VtcService', function($scope, $http, $location, VtcService){
  ...
  $scope.getAnnonce = function(id){
    VtcService.init().then(function(user){
      $scope.user = user;
      VtcService.get(id).then(function(annonce){
  ...
  $scope.getMessages = function(){
    VtcService.getMessages($scope.annonce).then(function(rep){
      if(rep.length>0){
        $scope.messages = rep;
  ...
  $scope.sendMessage = function(){
    VtcService.sendMessage($scope.message, $scope.annonce).then(function(rep){
  ...
  $scope.removeMessage = function(messageId){
    VtcService.removeMessage($scope.annonce, $scope.message, messageId).then(function(rep){
      $scope.getMessages();
    }, function(err){
      console.log(err);
    });
...
.controller("vtcReservationCtrl", ['$scope', 'VtcService', 'UserService','GoogleDistanceAPI', function($scope, VtcService, UserService, GoogleDistanceAPI){
  ...
  $scope.isDispo = function(id){
    ...
    VtcService.init().then(function(user){
      $scope.user = user;
      VtcService.isDispo(id, $scope.disponnibilite).then(function(data){
...
.controller('vtcAjoutCtrl', ['$scope',function($scope){
	...
	$scope.loadPlace = function($type){
		...
		// console.log(place);
		document.getElementById('annoncebundle_vtc_'+$type+'_latitude').value = place.geometry.location.lat();
		document.getElementById('annoncebundle_vtc_'+$type+'_longitude').value = place.geometry.location.lng();
		for (var i = 0; i < place.address_components.length; i++) {
			var addressType = place.address_components[i].types[0];
			if (componentForm[addressType]) {
				var val = place.address_components[i][componentForm[addressType]];
				document.getElementById('annoncebundle_vtc_'+$type+'_'+addressType).value = val;
			}
		}
	};
...
.controller('vtcModifCtrl', ['$scope','VtcService',function($scope, VtcService){
